...
async function main() {
  const baseUrl =
    "https://search.daum.net/search?w=news&nil_search=btn&DA=NTB&enc=utf8&cluster=y&cluster_page=1&q=금융서비스";
  let url = baseUrl;
  const res = await axios.get(url);
  const $ = cheerio.load(res.data);
  fs.writeFile("sample.html", res.data, (err) => {
    console.error(err);
  });
  ...
  const result = $contentTagArray
    .map((i, el) => {
      ...
      const imgUrl = $(el).find(".wrap_thumb img").prop("data-original-src");
      return {
        press,
        title,
        desc,
